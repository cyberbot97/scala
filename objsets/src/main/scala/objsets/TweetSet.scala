...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweetedAcc(tweet: Tweet): Tweet = {
    if (tweet.retweets > elem.retweets) {
      left.mostRetweetedAcc(right.mostRetweetedAcc(tweet))
    } else {
...
object GoogleVsApple {
  ...
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(tweet => google.exists(word => tweet.text.contains(word)))
