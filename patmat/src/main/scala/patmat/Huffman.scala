...
object Huffman {
  ...
  def chars(tree: CodeTree): List[Char] = tree match {
    case Fork(left, right, list, weight) => list
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case list1 :: list2 :: tail => (makeCodeTree(list1, list2) :: tail).sortWith((t1, t2) => weight(t1) < weight(t2))
    case _ => trees
  }
  ...
  def convert(tree: CodeTree): CodeTable = tree match {
    ...
    case Fork(left, right, chars, weights) => mergeCodeTables(convert(left), convert(right))
  }
